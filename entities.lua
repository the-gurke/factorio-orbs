...
-- Helper functions for wooden transport belt
local function tint_animation_layers(layers, tint)
...
local function tint_sprite_animations(sprites, tint)
    if not sprites then return end
    
    if sprites.sheet then
        sprites.sheet.tint = tint
    elseif sprites.sheets then
        for i, sheet in pairs(sprites.sheets) do
            sheet.tint = tint
        end
    elseif sprites.layers then
        for i, layer in pairs(sprites.layers) do
            layer.tint = tint
            tint_sprite_animations(layer, tint)
        end
    else
        sprites.tint = tint
    end
...
-- Create wooden transport belt entity
local wooden_tint = {r = 0.5, g = 0.3, b = 0.0, a = 1}
local wooden_belt = util.table.deepcopy(data.raw["transport-belt"]["transport-belt"])

wooden_belt.name = "wooden-transport-belt"
wooden_belt.minable.result = "wooden-transport-belt"
wooden_belt.max_health = 50
wooden_belt.speed = wooden_belt.speed * 0.5

wooden_belt.resistances = {
  {
    type = "fire",
    percent = -50
  },
...
if wooden_belt.belt_animation_set and wooden_belt.belt_animation_set.animation_set then
    if wooden_belt.belt_animation_set.animation_set.layers then
        tint_animation_layers(wooden_belt.belt_animation_set.animation_set.layers, wooden_tint)
    else
        wooden_belt.belt_animation_set.animation_set.tint = wooden_tint
    end
end

if wooden_belt.belt_horizontal_animation then
    if wooden_belt.belt_horizontal_animation.layers then
        tint_animation_layers(wooden_belt.belt_horizontal_animation.layers, wooden_tint)
    else
        wooden_belt.belt_horizontal_animation.tint = wooden_tint
    end
end

if wooden_belt.belt_vertical_animation then
    if wooden_belt.belt_vertical_animation.layers then
        tint_animation_layers(wooden_belt.belt_vertical_animation.layers, wooden_tint)
    else
        wooden_belt.belt_vertical_animation.tint = wooden_tint
    end
end

if wooden_belt.animations then
    for direction, animation in pairs(wooden_belt.animations) do
        tint_sprite_animations(animation, wooden_tint)
    end
end

if wooden_belt.connector_frame_sprites then
    tint_sprite_animations(wooden_belt.connector_frame_sprites, wooden_tint) 
end

if wooden_belt.circuit_connector_sprites then
    for i, sprite in pairs(wooden_belt.circuit_connector_sprites) do
        tint_sprite_animations(sprite, wooden_tint)
    end
end

data:extend({wooden_belt})

-- Create wooden pipe entity
local wooden_pipe = util.table.deepcopy(data.raw["pipe"]["pipe"])
wooden_pipe.name = "wooden-pipe"
wooden_pipe.minable.result = "wooden-pipe"
wooden_pipe.max_health = 50

-- Apply wooden tint to all pipe graphics
if wooden_pipe.pictures then
    tint_sprite_animations(wooden_pipe.pictures, wooden_tint)
end

-- Tint pipe covers if they exist
if wooden_pipe.covers then
    tint_sprite_animations(wooden_pipe.covers, wooden_tint)
end

-- Tint horizontal and vertical window backgrounds if they exist
if wooden_pipe.horizontal_window_background then
    tint_sprite_animations(wooden_pipe.horizontal_window_background, wooden_tint)
end
if wooden_pipe.vertical_window_background then
    tint_sprite_animations(wooden_pipe.vertical_window_background, wooden_tint)
end

-- Tint working visualizations if they exist
if wooden_pipe.working_visualisations then
    for _, vis in pairs(wooden_pipe.working_visualisations) do
        if vis.animation then
            tint_sprite_animations(vis.animation, wooden_tint)
        end
    end
end

-- Create wooden pipe to ground entity
local wooden_pipe_to_ground = util.table.deepcopy(data.raw["pipe-to-ground"]["pipe-to-ground"])
wooden_pipe_to_ground.name = "wooden-pipe-to-ground"
wooden_pipe_to_ground.minable.result = "wooden-pipe-to-ground"
wooden_pipe_to_ground.max_health = 50
wooden_pipe_to_ground.underground_distance = 5  -- Shorter distance than regular pipe to ground (10)

-- Apply wooden tint to all pipe to ground graphics
if wooden_pipe_to_ground.pictures then
    tint_sprite_animations(wooden_pipe_to_ground.pictures, wooden_tint)
end

-- Tint disabled visualization if it exists
if wooden_pipe_to_ground.disabled_visualization then
    tint_sprite_animations(wooden_pipe_to_ground.disabled_visualization, wooden_tint)
end

-- Tint working visualizations if they exist
if wooden_pipe_to_ground.working_visualisations then
    for _, vis in pairs(wooden_pipe_to_ground.working_visualisations) do
        if vis.animation then
            tint_sprite_animations(vis.animation, wooden_tint)
        end
    end
...
-- Tint fluid box pipe covers if they exist
if wooden_pipe_to_ground.fluid_box and wooden_pipe_to_ground.fluid_box.pipe_covers then
    tint_sprite_animations(wooden_pipe_to_ground.fluid_box.pipe_covers, wooden_tint)
end

data:extend({wooden_pipe, wooden_pipe_to_ground})
